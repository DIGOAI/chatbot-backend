...
from src.common.models import Conversation, GenericResponse, create_response
from src.common.models.conversation import ConversationStatus, ConversationWithData
from src.common.services.mock_data import clients, conversations, messages
...
def get_conversations(limit: int = 10):
    return create_response(conversations[:limit], f"Last {limit} conversations")


@router.get("/resume", response_model=GenericResponse[ConversationResume])
def get_conversation_resume():
    print("LLego aqui")
    opened = len([c for c in conversations if c.status == ConversationStatus.OPENED])
    closed = len([c for c in conversations if c.status == ConversationStatus.CLOSED])
...
def get_conversation(conversation_id: UUID):
    ...
    conversation_with_data.messages = [m for m in messages if m.conversation_id == conversation_id]
    conversation_with_data.messages.sort(key=lambda m: m.created_at)
    conversation_with_data.client = next((c for c in clients if c.id == conversation.client_id), None)
